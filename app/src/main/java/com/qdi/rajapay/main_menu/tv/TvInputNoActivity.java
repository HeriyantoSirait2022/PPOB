 ...
 * @screen 4.18.2
 */
public class TvInputNoActivity extends BaseActivity {
    ImageView image, image_provider;
    EditText no;
    ...
    private void init() throws JSONException {
        ...
        image_provider = findViewById(R.id.tv_provider);
        title_provider = findViewById(R.id.tv_name);
        ...
        title_provider.setText(data_provider.getString("text"));

        /**
         * @author : Jesslyn
         * @note : fixing picasso image to small
         */
        // <code>
        Picasso.get()
                .load(data_provider.getString("image_url"))
        ...
        add_ons_view_array.get(0).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(TvInputNoActivity.this, TvPriceListActivity.class)
    ...
    private void get_information() throws JSONException {
        ...
        add_key_value_information("ID PEL",data.getString("noTv"));
        ...
        for(int x = 0; x < data.getJSONArray("tagihan").length(); x++){
            ...
            add_key_value_detail_information("Periode " + (x + 1), OrderData.isNullThenConvert(detail.getString("periode")) );

            if(detail.has("jatuhTempo"))
                add_key_value_detail_information("- Jatuh Tempo", OrderData.isNullThenConvert( detail.getString("jatuhTempo")) );
            add_key_value_detail_information("- Ref", OrderData.isNullThenConvert(detail.getString("referensi")) );
            if(detail.has("noref1"))
                add_key_value_detail_information("- No Ref 1", OrderData.isNullThenConvert(detail.getString("noref1")) );
            if(detail.has("noref2"))
                add_key_value_detail_information("- No Ref 2", OrderData.isNullThenConvert(detail.getString("noref2")));
            if(detail.has("paket"))
                add_key_value_detail_information("- Paket", OrderData.isNullThenConvert(detail.getString("paket")));
    ...
    private void get_data() throws JSONException {
        url = BASE_URL+"/mobile/postpaid/tv-info";
        show_wait_modal();

        JSONObject param = getBaseAuth();
        param.put("productType", "TVSUB");
        param.put("noTv", no.getText().toString() );
    ...
    private void redirect_page() throws JSONException {
         ...
         * @author Jesslyn
         * @note 1. add detail
         *       2. remove to lower case 11/22/2020 2:12 - Case 9 - design
         */
        // <code>
        ...
        data1.put("url_post","/mobile/postpaid/tv-transaction");
        data1.put("url_pay","/mobile/postpaid/tv-payment");

        startActivity(new Intent(TvInputNoActivity.this, MainMenuConfirmationActivity.class)
                .putExtra("type","tv")
    ...
    private JSONObject add_data_to_post() throws JSONException {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("noTv",data.getString("noTv"));
        ...
        jsonObject.put("typeTxn","POSTPAIDTV");
