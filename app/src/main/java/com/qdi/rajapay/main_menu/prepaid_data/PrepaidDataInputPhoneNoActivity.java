 ...
 * @screen 4.10.5
 */
public class PrepaidDataInputPhoneNoActivity extends BaseActivity {
    ...
    JSONObject data_selected, selected_provider = new JSONObject(), data_list = new JSONObject();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_menu_prepaid_data_input_phone_no);

        init_toolbar(getResources().getString(R.string.home_mobile_data));
        try {
            init();
        } catch (JSONException e) {
            e.printStackTrace();
                ...
                try {
                    ...
                    if(selected_tab.equalsIgnoreCase("NGRS")){
                        ...
                        if(data_selected.getInt("isProblem") == 1)
                            ...
                            PrepaidDataInputPhoneNoModal modal = new PrepaidDataInputPhoneNoModal();
    ...
    private void check_provider() throws JSONException {
        ...
        for(int x = 0; x < provider_array.size(); x++){
            ...
            for(int y = 0; y < prefix_array.length; y++){
                if(input.length() >= prefix_array[y].length() && check_phone_no(prefix_array[y],
                        input.substring(0,input.length() < prefix_array[y].length() ? input.length() : prefix_array[y].length())
                )) {
    ...
    private void get_provider_data() {
        url = BASE_URL+"/mobile/prepaid-data/show-operator-prefix";

        JSONObject param = getBaseAuth();
        try{
            param.put("productType", "DATA");
        }catch(Exception e){
            e.printStackTrace();
        }
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    if (!response_data.getString("type").equals("Failed")) {
                        JSONArray jsonArray = response_data.getJSONArray("arrDataPrefix");
    ...
    private void get_package_data() throws JSONException {
        ...
        url = BASE_URL+"/mobile/prepaid/data-pricelist";
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    if (!response_data.getString("type").equals("Failed")) {
                        ...
                        JSONObject resp = response_data.getJSONObject("arrDataPriceRslt");
                        ...
                        while(keys.hasNext()) {
                            ...
                            if (resp.getJSONArray(key) instanceof JSONArray) {
                                ...
                                for(int j = 0; j < arr.length(); j++){
                                    ...
                                    jsonObject.put("name",jsonObject.getString("nameData"));
                                    jsonObject.put("detail",jsonObject.getString("detailData"));
                                    // <code>
    ...
    private void get_detail_ngrs(final int index_ngrs) throws JSONException {
        ...
        url = BASE_URL+"/mobile/prepaid/data-ngrs-check";
        ...
        try{
            ...
            param.put("codeData", selectedCat.get(index_ngrs).getString("codeData") );
            param.put("nameData", selectedCat.get(index_ngrs).getString("nameData") );
            param.put("typeData", selectedCat.get(index_ngrs).getString("typeData") );
            param.put("codeCheckData", selectedCat.get(index_ngrs).getString("codeCheckData") );
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    if (!response_data.getString("type").equals("Failed")) {
                         ...
                         *         2. change to vendorAmount (to make standardization code)
                         */
                        // <code>
